...
public class ClientTcpNetwork extends ClientNetwork implements Runnable {
    private final ClientTcpConnection[] connections;
     ...
     */
    public ClientTcpNetwork() throws IOException {
        this.p = ProcessDescriptor.getInstance();
        this.connections = new ClientTcpConnection[p.numReplicas];
        logger.fine("Opening port: " + p.getLocalProcess().getReplicaPort());
        this.server = new ServerSocket();
        server.setReceiveBufferSize(256 * 1024);
        server.bind(new InetSocketAddress((InetAddress) null, p.getLocalProcess().getClientPort())); //.getReplicaPort()
    ...
    public void start() {
        if (!started) {
        	/* Disabled by Utkarsh for script */
		System.out.println("Connections = " + connections.length);
            for (int i = 0; i < connections.length; i++) {
                if (i < p.localId) {
                    connections[i] = new ClientTcpConnection(this, p.config.getProcess(i), false);
                    connections[i].start();
                }
                if (i > p.localId) {
                    connections[i] = new ClientTcpConnection(this, p.config.getProcess(i), true);
                    connections[i].start();
                }
            }
    ...
    public void sendMessage(ClientMessage message, BitSet destinations) {
    ...
    public void sendMessage(ClientMessage message, int destination) {
